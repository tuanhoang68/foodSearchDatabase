...
def RGB_to_Gray(image_path): # Chuyển từ ảnh màu về ảnh xám
    ...
    # Tiếp theo, đoạn code sử dụng một vòng lặp để lặp qua từng pixel của ảnh màu và tính toán giá trị trung 
    # bình có trọng số của các kênh R, G và B tại mỗi pixel. Giá trị trung bình này được gán cho mỗi pixel 
    # tương ứng trong mảng grayImage. 
    for i in range(3):
        grayImage[:, :, i] = Avg
    
    print("grayImage: ", grayImage[0][0])
    print(len(grayImage))
...
def GetCoMatrix(img_zip):
    gray_img = color.rgb2gray(img_zip)
    ...
    matrix_coocurrence = skimage.feature.graycomatrix(image, distances=[1], angles=[0, np.pi/4, np.pi/2, 3*np.pi/4], levels=256, normed=True, symmetric=False)
