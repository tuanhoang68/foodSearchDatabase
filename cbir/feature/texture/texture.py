...
def GetCoMatrix(img_zip):
    # Hàm có tên GetCoMatrix lấy một hình ảnh làm đầu vào và trả về một ma trận cùng xuất hiện. 
    # Dưới đây là giải thích từng bước về chức năng của code:

    # 1. Hàm color.rgb2gray từ thư viện skidage được sử dụng để chuyển đổi hình ảnh đầu vào thành thang độ xám. 
    # Hình ảnh thang độ xám kết quả được lưu trữ trong biến màu xám.
    gray = color.rgb2gray(img_zip)
    
    # Chuyển đổi hình ảnh thang độ xám thành tập các số nguyên không dấu 8 bit.
    image = img_as_ubyte(gray)
    
    # 5. Hàm skiage.feature.graycomatrix được sử dụng để tính toán ma trận cùng xuất hiện. 
    matrix_coocurrence = skimage.feature.graycomatrix(image, distances=[1], angles=[0, np.pi/4, np.pi/2, 3*np.pi/4], levels=256, normed=False, symmetric=False)
    
    # 6. Ma trận đồng xuất hiện được trả về từ hàm.
    return matrix_coocurrence

    # Nhìn chung, hàm này lấy một hình ảnh đầu vào, chuyển đổi nó thành thang độ xám, tính toán biểu đồ 
    # giá trị cường độ và sau đó tính toán ma trận cùng xuất hiện dựa trên biểu đồ. 
    # Ma trận cùng xuất hiện kết quả có thể được sử dụng làm tính năng kết cấu cho các nhiệm vụ phân tích hình ảnh.
...
def getTextureMatrix(image_path):
    ...
    energy          =   base.normalizationVector(energy_feature(matrix_coocurrence))
    contrast        =   base.normalizationVector(contrast_feature(matrix_coocurrence))
    dissimilarity   =   base.normalizationVector(dissimilarity_feature(matrix_coocurrence))
    homogeneity     =   base.normalizationVector(homogeneity_feature(matrix_coocurrence))
    correlation     =   base.normalizationVector(contrast_feature(matrix_coocurrence))
...
def main():
    image_path = "CBIR/tests/image_test/ff.jpg"
    print(getTextureMatrix(image_path))
    print(len(getTextureMatrix(image_path)))
